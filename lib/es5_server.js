...
main.use('/sprints', function (req, res, next) {
    if (req.originalUrl == "/sprints"){
...
main.use('/all', function (req, res, next) {
    controller.listAllSprints(req, res);
    return;
});
...
main.use('/api/sprint', function (req, res, next) {
    ...
    if (req.method === "GET" && req.query.sprint) {
        ...
        var result = {
            ...
            dailyMeeting: springConfig.dailyMeeting,
        ...
        for (var i = 0; i < springConfig.days.length; i++) {
            result.days.push({ day: springConfig.days[i], isWorkDay: springConfig.resources[i] === '1' ? true : false, include: true });
        }
...
function getConfigFromJson(POST) {
    var config = {
        ...
        dailyMeeting: POST.dailyMeeting,
        boardId: POST.boardId,
        name: POST.name
    };
    
    var dayIndex = 0;
    for (var i = 0; i < POST.days.length; i++) {
        if (POST.days[i].include) {
            config.days[dayIndex] = POST.days[i].day;
            config.resources[dayIndex] = POST.days[i].isWorkDay ? '1' : '0';
            dayIndex++;
        }
    }
